...
const admins = computed(() => props.users.map(admin => ({ ...admin, school_id: admin.school.id, password: '' })))
...
<template>
  ...
  >
    <template #deleteMessage="{item}">
      <b class="text-[1.1rem] text-gray-900">Czy na pewno chcesz usunąć "{{ item.name }} {{ item.surname }}"?</b>
